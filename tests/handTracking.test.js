...
import { PADDLE } from '../src/constants.js';
...
test('larger hand paddle has a matching hit area and restores the controller geometry', (t) => {
  cameraEnvironment(t);
  const paddle = new Paddle();
  const blade = paddle.mesh.getObjectByName('blade');
  const originalPosition = blade.position.clone();
  const originalRotation = blade.quaternion.clone();
  paddle.setPalmTrackingMode(true);
  paddle.updateFromCamera(1000);
  paddle.updateFromCamera(1033);
  close(paddle.mesh.scale.x, 1.4);
  close(paddle.headRadius, PADDLE.HEAD_RADIUS * 1.4);
  close(paddle.headThickness, PADDLE.HEAD_THICKNESS * 1.4);

  const physics = new PhysicsWorld();
  const strike = (radius) => {
    const ball = {
      mesh: { position: new THREE.Vector3(radius, 0, -0.1) },
      velocity: new THREE.Vector3(0, 0, -3),
      spin: new THREE.Vector3(), touchedByPaddle: false,
    };
    physics._collidePaddle(ball, paddle, new THREE.Vector3(radius, 0, 0.1));
    return ball.touchedByPaddle;
  };
  assert.equal(strike(PADDLE.HEAD_RADIUS * 1.2), true);
  assert.equal(strike(PADDLE.HEAD_RADIUS * 1.5), false);
  paddle.resetTracking();
  assert.equal(strike(0), false, 'a lost hand must not hit the ball');
  paddle.setPalmTrackingMode(false);
  close(paddle.mesh.scale.x, 1);
  close(paddle.headRadius, PADDLE.HEAD_RADIUS);
  close(paddle.headThickness, PADDLE.HEAD_THICKNESS);
  assert.ok(blade.position.equals(originalPosition));
  assert.ok(blade.quaternion.equals(originalRotation));
});
